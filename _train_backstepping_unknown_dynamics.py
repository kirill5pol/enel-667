...
CONST_ETA = 20000  # The value of k_u_alpha_dot = 49.1493074 in the linearized version
# For the full system I got this: 9829.27 as the maximum value of k_u_alpha_dot
# (from wolfram alpha: https://tinyurl.com/y3kgcn38)


def generate_unkown_dyn_labels(xs):
    N, D = xs.shape
    ...
    ys = np.zeros((N, 2))
    ys[:, 0] = (
        theta_dot_dots - alphas
    )  # f2 = -x3 + theta_dot_dot = theta_dot_dot - alpha
    ys[:, 1] = alpha_dot_dots - CONST_ETA * taus  # f4 = -eta*u + alpha_dot_dot
    return ys


def from_unknown_dyn_labels_to_derivatives(xs, ys):
    N, D = xs.shape
    ...
    d_state[:, 2] = ys[:, 0] + alphas  # θ̈ = f2 + α
    d_state[:, 3] = ys[:, 1] + CONST_ETA * alphas  # α̈ = f4 + η*u
    return d_state
...
def generate_training_set(N, dist_type="uniform"):
    ...
    ys = generate_unkown_dyn_labels(xs)
    return xs, ys
...
def main():
    ...
    xs, ys = generate_training_set(ne, dist_type="normal")
    N, D = xs.shape
    Ny, M = ys.shape

    # Generate a testing set for looking at the mean percent error
    xs_test, ys_test = generate_training_set(int(0.5 * ne), dist_type="normal")
